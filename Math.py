...
class Calculator(object):
	...
	def predictedCrossingsForDefenseCategory(self, team, category):
		predCrossings = [team.calculatedData.predictedSuccessfulCrossingsForDefenseTele[dKey] for dKey in self.defenseDictionary[category] if self.teamFacedDefense(team, dKey) and team.calculatedData.predictedSuccessfulCrossingsForDefenseTele[dKey] != None] # TODO: Update with actual correct key
		return np.mean(predCrossings) if len(predCrossings) > 0 else 0
	...
	def standardDeviationForTeamForCategory(self, team, category):
		...
		return utils.rms(map(lambda dKey: sumDefenseCrossingsDict[dKey] if (dKey in sumDefenseCrossingsDict.keys()) else 0, filter(lambda d: self.teamFacedDefense(team, d), self.defenseDictionary[category])))

	def autoCrossingsForCategory(self, team, category):
		return np.mean([(team.calculatedData.avgSuccessfulTimesCrossedDefensesAuto[defense] if defense in team.calculatedData.avgSuccessfulTimesCrossedDefensesAuto.keys() else 0.0) or 0.0 for defense in self.defenseDictionary[category]])

	def stdAutoCrossingsForCategory(self, team, category):
		return utils.rms(filter(lambda x: x != None, [(team.calculatedData.sdSuccessfulDefenseCrossesAuto[defense] if defense in team.calculatedData.sdSuccessfulDefenseCrossesAuto else 0.0) for defense in self.defenseDictionary[category]]))

	def getDefenseDamageChanceForAllianceForCategory(self, alliance, category):
		crossings = sum(map(lambda t: self.predictedCrossingsForDefenseCategory(t, category) if self.predictedCrossingsForDefenseCategory(t, category) != None else 0, alliance))
	...
	def winChanceForMatchForAllianceIsRed(self, match, allianceIsRed):
		...
		tscoreRPs = self.welchsTest(predictedScore,
									   ...
									   sampleSize,
									   opposingSampleSize)
		cdf = stats.t.cdf(tscoreRPs, np.mean([sampleSize, opposingSampleSize]))
		return cdf if not np.isnan(cdf) else 0
...
'''
    ...
    #     crossesDict = timd.timesSuccessfulCrossedDefensesAuto or {}
    ...
    #     for combo in self.defenseCombos:
    ...
    def doMatchesCalculations(self):
        self.defenseCombos = self.defenseCombinations()
